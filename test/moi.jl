...
function moi_01(optimizer)
    ...
    c1 = MOIU.normalize_and_add_constraint(optimizer, 
        MOI.ScalarAffineFunction([
            MOI.ScalarAffineTerm(2.0, X[1]),
            MOI.ScalarAffineTerm(1.0, X[2])
        ], 0.0), MOI.EqualTo(4.0))

    c2 = MOIU.normalize_and_add_constraint(optimizer, 
        MOI.ScalarAffineFunction([
            MOI.ScalarAffineTerm(1.0, X[1]),
            MOI.ScalarAffineTerm(2.0, X[2])
        ], 0.0), MOI.EqualTo(4.0))

    b1 = MOIU.normalize_and_add_constraint(optimizer, 
        MOI.ScalarAffineFunction([
            MOI.ScalarAffineTerm(1.0, X[1])
        ], 0.0), MOI.GreaterThan(0.0))

    b2 = MOIU.normalize_and_add_constraint(optimizer, 
        MOI.ScalarAffineFunction([
            MOI.ScalarAffineTerm(1.0, X[2])
        ], 0.0), MOI.GreaterThan(0.0))
...
function moi_02(optimizer, mode = BilevelJuMP.SOS1Mode{Float64})
    ...
    c1 = MOIU.normalize_and_add_constraint(lower, 
        MOI.ScalarAffineFunction([
    ...
    c2 = MOIU.normalize_and_add_constraint(lower, 
        MOI.ScalarAffineFunction([
    ...
    b1 = MOIU.normalize_and_add_constraint(lower, 
        MOI.ScalarAffineFunction([
            MOI.ScalarAffineTerm(1.0, X2[1])
        ], 0.0), MOI.GreaterThan(0.0))
    MOI.set(lower, MOI.ConstraintName(), b1, "l_b1")

    b2 = MOIU.normalize_and_add_constraint(lower, 
        MOI.ScalarAffineFunction([
            MOI.ScalarAffineTerm(1.0, X2[2])
        ], 0.0), MOI.GreaterThan(0.0))
    ...
    blp, _, _ = BilevelJuMP.build_bilivel(upper, lower, links, parametric, mode)
...
function moi_03(optimizer, mode = BilevelJuMP.SOS1Mode{Float64})
    ...
    c3 = MOIU.normalize_and_add_constraint(lower, 
    MOI.ScalarAffineFunction([
        MOI.ScalarAffineTerm(1.0, x2),
