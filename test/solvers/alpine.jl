...
const ALP_IPOPT = optimizer_with_attributes(
    Ipopt.Optimizer,
    MOI.Silent() => true,
    "sb" => "yes",
)
...
push!(
    solvers_nlp,
    (opt = ALP, mode = BilevelJuMP.ProductMode(1e-9; with_slack = true)),
)
...
push!(
    solvers_nlp_sd,
    (opt = ALP, mode = BilevelJuMP.StrongDualityMode(1e-9; inequality = true)),
)
push!(
    solvers_nlp_sd,
    (opt = ALP, mode = BilevelJuMP.StrongDualityMode(; inequality = false)),
)
push!(
    solvers_nlp_sd_i,
    (opt = ALP, mode = BilevelJuMP.StrongDualityMode(1e-9; inequality = true)),
