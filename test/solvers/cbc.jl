...
MOI.set(CBC_OPTIMIZER, MOI.TimeLimitSec(), 60)
...
push!(
    solvers_fa,
    (opt = CBC, mode = BilevelJuMP.FortunyAmatMcCarlMode(; with_slack = false)),
)
push!(
    solvers_fa,
    (opt = CBC, mode = BilevelJuMP.FortunyAmatMcCarlMode(; with_slack = true)),
)
push!(
    solvers_fa2,
    (
...
CBC_CACHED = MOIU.CachingOptimizer(
...
push!(
    ...
    (opt = QTB_CBC_BRIDGED, mode = BilevelJuMP.SOS1Mode()),
)
